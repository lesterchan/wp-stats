...
function get_recentposts( $mode = '', $limit = 10, $display = true ) {
	...
	if ( $recentposts ) {
		foreach ( $recentposts as $post ) {
			...
			$post_date    = get_the_time( sprintf( __( '%1$s @ %2$s', 'wp-stats' ), get_option( 'date_format' ), get_option( 'time_format' ) ) );
...
function get_recentcomments( $mode = '', $limit = 10, $display = true ) {
	...
	if ( $recentcomments ) {
		foreach ( $recentcomments as $post ) {
			...
			$comment_date   = mysql2date( sprintf( __( '%1$s @ %2$s', 'wp-stats' ), get_option( 'date_format' ), get_option( 'time_format' ) ), $post->comment_date );
...
function stats_page() {
	...
	if ( empty( $comment_author ) ) {
		...
		$temp_stats  .= '<p>' . sprintf( __( 'Displaying <strong>%1$s</strong> To <strong>%2$s</strong> Of <strong>%3$s</strong> Comments', 'wp-stats' ), number_format_i18n( $display_on_page ), number_format_i18n( $max_on_page ), number_format_i18n( $numposts ) ) . '</p>';
		...
		if ( $max_page > 1 ) {
			...
			$temp_stats .= '<span class="pages">&#8201;' . sprintf( __( 'Page %1$s of %2$s', 'wp-stats' ), number_format_i18n( $page ), number_format_i18n( $max_page ) ) . '&#8201;</span>';
